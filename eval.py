...
def clean_data(gt_anno, dt_anno, current_class, difficulty, depth_limit=(0, float("Inf"))):
    ...
    for i in range(num_gt):
        ...
        if gt_name == current_cls_name:
            ...
            current_cls_name == "Pedestrian".lower() and "Person_sitting".lower() == gt_name
        ):
            valid_class = 0
        ...
        if (
            ...
            or ((depth < depth_limit[0]) or (depth > depth_limit[1]))
        ):
    ...
    for i in range(num_dt):
        ...
        if height < MIN_HEIGHT[difficulty] or (
            (depth < depth_limit[0]) or (depth > depth_limit[1])
        ):
            ignored_dt.append(1)
        elif valid_class == 1:
            ignored_dt.append(0)
...
def _prepare_data(
    gt_annos, dt_annos, current_class, difficulty, depth_limit=(0, float("Inf"))
):
    gt_datas_list = []
    dt_datas_list = []
    total_dc_num = []
    ...
    for i in range(len(gt_annos)):
        rets = clean_data(
            gt_annos[i], dt_annos[i], current_class, difficulty, depth_limit=depth_limit
        )
...
def eval_class(
    ...
    for m, current_class in enumerate(current_classes):
        for l, difficulty in enumerate(difficultys):
            rets = _prepare_data(
                gt_annos, dt_annos, current_class, difficulty, depth_limit=depth_limit
            )
            (
                gt_datas_list,
                dt_datas_list,
...
def get_official_eval_result(
    gt_annos, dt_annos, current_classes, PR_detail_dict=None, depth_limit=(0, float("Inf"))
):
        ...
        for i in range(min_overlaps.shape[0]):
            ...
            if compute_aos:
                ...
                if i == 0:
                    ...
                    ret_dict["%s_aos/moderate_R40" % class_to_name[curcls]] = mAPaos_R40[
                        j, 1, 0
                    ]
...
def get_coco_eval_result(gt_annos, dt_annos, current_classes):
    ...
    for j, curcls in enumerate(current_classes):
        ...
        result += print_str(
            (f"{class_to_name[curcls]} " "coco AP@{:.2f}:{:.2f}:{:.2f}:".format(*o_range))
        )
        result += print_str(
            (
        ...
        result += print_str(
            (f"bev  AP:{mAPbev[j, 0]:.2f}, " f"{mAPbev[j, 1]:.2f}, " f"{mAPbev[j, 2]:.2f}")
        )
        result += print_str(
